...
public partial class lawyer_Upcoming_appointments : System.Web.UI.Page
{
    ...
    public void fillAppointment()
    {
        ...
        foreach(DataRow dr in dt.Rows)
        {
            ...
            {
                html.Append("<td>Active / <a href='Javascript:closeBookingfunction(" + dr["appointment_id"].ToString() + ");'>Close Booking</a></td>");
    ...
    [System.Web.Services.WebMethod]
    public static string closeappointment(string eid)
    {
        try
        {
            DBConnectionClass con = new DBConnectionClass();

            bool i = con.boolInsertData("update LAS_appointment set a_status='2' where appointment_id='" + eid.ToString().Trim() + "'");
            if (i == true) return "true"; else return "false";
        }
        catch (Exception)
        {
